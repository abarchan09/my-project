...
from src.source import add_pv,add_grid,add_gas_source
from wsr.source import add_solar_thermie
from src.sink import add_heat_demand,add_export,add_heat_dump
from wsr.converter import waerme_tauscher,wasser_heat_pump
from src.converter import add_gas_boiler
...
def bauen():
    ...
    source_bus= solph.Bus(label="wasser")
    gas_bus= solph.Bus(label="gas")
    #-----------------------#
    sol_bus= solph.Bus(label="strahlung")
    es.add(el_bus,heat_bus,source_bus,gas_bus,sol_bus )    

    print("4) Source: pv,grid, heat_source")
    add_pv(es, el_bus, df)
    add_grid(es, el_bus,df)
    add_solar_thermie(es,sol_bus,df_sl)
    ...
    print("7) Heat pump ")
    wasser_heat_pump(es,el_bus,heat_bus,source_bus,cfg,df_sl)
    add_gas_boiler(es,gas_bus,heat_bus)
    waerme_tauscher(es,sol_bus,source_bus)


    

    print("✅ Build erfolgreich ")
    return es,df,df_sl, cfg
...
if __name__ == "__main__":
    ...
    if results is None:
        ...
        save_output_to_csv(output_data, "oemof_output_wsr")
