...
def extract_result_series(results) -> dict[str, pd.Series]:
    """
    Extrahiert alle benötigten Zeitreihen/Scalars aus oemof results genau einmal.
    Rückgabe: dict mit pandas.Series (Zeitreihen) und floats (Invest).
    """
...
def save_output_to_csv(output_data, filename_prefix="results"):
    ...
    df_timeseries.to_csv(f"{filename_prefix}_timeseries.csv", sep=";")

    # ---- Scalars speichern ----
    df_scalars = pd.DataFrame.from_dict(
        scalar_dict, orient="index", columns=["value"]
    )
    df_scalars.to_csv(f"{filename_prefix}_scalars.csv", sep=";")
